...
local HR = createButton("HallowenRoped")
local AS = createButton("Get Addons")
...
Weather.MouseButton1Click:Connect(function()
    ...
    ReplicatedStorage.SpawnObject:FireServer("bigbadblizzard")
end)
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if self == LocalPlayer and (method == "Kick" or method == "kick") then
        Notify("🚫 Anti-kick", "A Kick Command Blocked!", 5)
        return nil
    end
    return old(self, ...)
end)
...
LocalPlayer.Kick = function(...)
    Notify("🚫 Anti-Kick", "A direct kick command was blocked!", 5)
end
